...
#define kMinMaxWidth 45
...

    {
       r.top = 50 + yoff;
        ...
        r.left = xoff;
        r.right = r.left + 350;
        verify_noerr(CreateStaticTextControl(mCarbonWindow, &r, CFSTR("Johnny Petrantoni JACK-insert v1.6.2, (c) 2003 - 2007    ") , &fontStyle, &newControl));
        verify_noerr(EmbedControl(newControl));
        r.top = 75 + yoff;
        r.bottom = r.top + 80;
        r.left = xoff;
        r.right = r.left + 350;
        verify_noerr(CreateStaticTextControl(mCarbonWindow, &r, CFSTR("johnny@meskalina.it    ") , &fontStyle, &newControl));
        verify_noerr(EmbedControl(newControl));
        r.top = 100 + yoff;
        r.bottom = r.top + 80;
        r.left = xoff;
